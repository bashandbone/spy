...
// at row 0, (b) the wrap cache invalidates, (c) the next paint
// completes in ≤ 16 ms p95 across 50 successive resize events at
// random widths in [40, 200] columns.
//
...
func TestResize_PreservesViewportAnchor(t *testing.T) {
	...
	// Deterministic random sequence so failures are reproducible.
	...
	if p95 > limit {
		t.Fatalf("SC-008: resize p95 %v exceeds %v budget across %d iterations",
