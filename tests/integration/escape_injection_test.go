...
// TestEscapeInjection_OSCSequenceNeutralised is the SC-T109b.c gate:
// a file whose bytes contain `\x1b]2;malicious\x07` (the OSC 2
// "set window title" sequence) must NOT have those bytes survive
// through to the rendered output. The renderer neutralises ESC bytes
...
func TestEscapeInjection_OSCSequenceNeutralised(t *testing.T) {
	...
	if strings.Contains(view, "\x1b]2;") {
		t.Errorf("rendered frame contains live OSC payload — escape was not neutralised; view tail=%q",
			view[max(0, len(view)-200):])
	}
	if !strings.Contains(view, "before") || !strings.Contains(view, "after") {
		t.Errorf("escape neutralisation dropped surrounding content; view=%q", view)
	}
	// No `\x1b]` sequences at all (DCS / OSC / SOS) — only SGR
	// escapes (`\x1b[...m`) from the renderer's chroma colour are
