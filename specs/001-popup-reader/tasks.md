...
- [X] T104 [P] Add SC-001 benchmark in `tests/perf/firstframe_bench_test.go`: load a 100-line file end-to-end and assert ≤ 150 ms from invocation at p95. *SC-001 budget updated from 100 ms to 150 ms (issue #20): spawn-based timing in `TestFirstFrame_Under150ms` is a hard gate; the gap to 100 ms is intrinsic binary-startup overhead (Go runtime ~30–50 ms + Chroma lexer registry init + glamour/goldmark). The `TestFirstFrame_RendererSlice` variant enforces a separate ≤ 20 ms p95 hard gate on the renderer slice so renderer regressions are caught independently of binary-startup jitter. See spec.md SC-001 trade-off note.*
