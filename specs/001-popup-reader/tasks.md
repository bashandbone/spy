...
- [X] T104 [P] Add SC-001 benchmark in `tests/perf/firstframe_bench_test.go`: load a 100-line file end-to-end and assert ≤ 150 ms from invocation. *SC-001 budget updated from 100 ms to 150 ms (issue #20): spawn-based timing in `TestFirstFrame_Under150ms` is a hard gate; the gap to 100 ms is intrinsic binary-startup overhead (Go runtime ~30–50 ms + Chroma lexer registry init + glamour/goldmark). The `TestFirstFrame_RendererSlice` variant remains a strict gate on the renderer slice (~12 ms p95). See spec.md SC-001 trade-off note.*
