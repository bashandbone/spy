...
- **SC-001**: Users can open and view a 100-line text file with syntax highlighting in under 150 ms from invocation. *Trade-off note: the renderer slice contributes ≤ 15 ms p95; the remaining ~100–130 ms is intrinsic binary-startup overhead — Go runtime initialisation (~30–50 ms on commodity Linux), Chroma lexer registry boot (257 embedded XML configs), and glamour/goldmark registration. Reducing this further would require lazy lexer loading or a persistent helper process; both are deferred beyond v0.1.0. The renderer-slice gate (`TestFirstFrame_RendererSlice`) remains at 15 ms so renderer regressions are caught independently of binary-startup jitter.*
...
- **SC-010**: For `dummy.pdf` (single-page, known text "Dummy PDF file") and `tests/fixtures/pdf/multi-page.pdf` (3 pages), the renderer (a) rasterizes page 1 via `internal/render.rasterizePDFPage` (backed by `github.com/gen2brain/go-fitz`) under `-tags fitz` in a graphics-capable PTY, and (b) extracts page text via `github.com/ledongthuc/pdf` in a non-graphics PTY — the extracted text MUST contain the known sentinel string. Measured by `tests/integration/pdf_test.go`. *(Acceptance review H6 — replaces earlier `pdfcpu` / `graphics.PDFPage` naming.)*
- **SC-011**: Non-TTY stdin/pipeline behavior passes end-to-end in `tests/e2e/05_pipe.sh`: the script pipes fixture content from `tests/e2e/fixtures/hello.go` into `spy` and verifies the verbatim non-interactive output and exit-code contracts for piped input — covering `cat | spy -l go`, bare `cat | spy`, `grep -n | spy`, the explicit `printf | spy | cat` degenerate-cat round-trip, the `-` positional, and the empty-stdin case. The interactive PTY-only half of SC-011 — the alt-screen footer literally rendering `<stdin>` rather than a basename, syntax-highlighted display of the streamed content, and `q` exiting cleanly — is validated separately by the PTY-driven integration tests at `tests/integration/stdin_test.go` and `tests/integration/footer_test.go`. *(Acceptance review M11 — split SC-011 into a non-TTY shell-scaffolded half and a PTY-driven integration half to match the harness reality.)*
